...
int main(int argc, char* argv[]) {
    ...
    char patterns[5][64];  // Array fisso per pattern (max. 5 pattern, max 63 caratteri)
    int pattern_count = 0;
    ...
    while ((opt = getopt(argc, argv, "l:p:i:vh")) != -1) {
        ...
        case 'p':
            ...
            strcpy(patterns[pattern_count], optarg);  // VULNERABILITY: Potenziale Buffer Overflow (se 6 o più pattern)
            pattern_count++;
            break;
        case 'i':
